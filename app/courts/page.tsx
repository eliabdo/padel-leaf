    ...
    <>
      ...
      <header className="bg-forest text-cream py-16 sm:py-24">
        <div className="max-w-7xl mx-auto px-5 sm:px-6">
          ...
          <h1 className="text-[2.25rem] sm:text-5xl md:text-6xl leading-[1.08] mb-5 sm:mb-6 max-w-3xl">
      ...
      <section className="bg-sage-soft py-14 sm:py-20">
        <div className="max-w-4xl mx-auto px-5 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 gap-5 sm:gap-6 mt-7 sm:mt-8">
...
function CourtTile({ name, tag, surface, description }: typeof COURTS[number]) {
  return (
    <div className="bg-cream rounded-2xl overflow-hidden border border-forest/10 hover:shadow-xl hover:-translate-y-1 transition-all">
      <div className="h-44 sm:h-56 bg-gradient-to-br from-forest-mid to-forest flex items-center justify-center">
...
function SpecRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between items-baseline gap-3 border-b border-forest/15 py-3">
      <span className="text-char-soft text-sm shrink-0">{label}</span>
      <span className="font-serif text-forest-deep text-right text-sm sm:text-base">{value}</span>
    </div>
  );
}
